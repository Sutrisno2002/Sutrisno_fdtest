<div class="py-1 px-4" aria-label="Pagination Navigation">
    @if ($paginator->hasPages())
        <nav class="pagination">
            ...
            @else
                <a class="w-6 h-6 text-gray-400 hover:text-dark p-2 flex items-center gap-2 font-medium rounded-md"
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <a class="w-6 h-6 bg-dark text-white p-2 flex justify-center items-center text-xs font-medium rounded-full"
                               href="javascript:void(0)" aria-current="page"
                               wire:key="paginator-{{ $paginator->getPageName() }}-page-{{ $page }}">
                                {{ $page }}
                            </a>
                        @else
                            <a class="w-6 h-6 text-gray-400 hover:text-dark p-2 flex justify-center items-center text-xs font-medium rounded-full"
